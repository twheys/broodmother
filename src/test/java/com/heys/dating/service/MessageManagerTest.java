...
import com.heys.dating.domain.repository.impl.MemberMessageRepositoryDatastoreImpl;
import com.heys.dating.domain.repository.impl.MemberRepositoryDatastoreImpl;
import com.heys.dating.domain.repository.impl.MemberThreadRepositoryDatastoreImpl;
import com.heys.dating.domain.repository.impl.MessageRepositoryDatastoreImpl;
import com.heys.dating.domain.repository.impl.ProfileRepositoryDatastoreImpl;
import com.heys.dating.domain.repository.impl.ThreadRepositoryDatastoreImpl;
...
import com.heys.dating.manager.impl.MessageManagerImpl;
...
import com.heys.dating.service.impl.MessageTransformerImpl;
import com.heys.dating.test.TestQueueManager;
import com.heys.dating.util.DatastoreUtil;

public class MessageManagerTest extends MemberTest {
	...
	private void assertMessages(final Member recipient, final Thread thread,
		...
		final ThreadDTO threadDTO = messageMgr.getMessagesForThread(recipient,
				thread, 10, 0);
	...
	private void assertThreadPreview(final Member lastSender,
		...
		final ThreadListDTO threadListDTO = messageMgr.getNewThreadsForMember(
				recipient, 10, 0);
	...
	private Message sendMessage(final Member sender, final String subject,
		...
		if (directMessage && subjectPresent) {
			message = messageMgr.send(sender, participants[0], subject, text);
		} else if (directMessage) {
			message = messageMgr.send(sender, participants[0], text);
		} else if (subjectPresent) {
			message = messageMgr.send(sender, Lists.newArrayList(participants),
					subject, text);
		} else {
			message = messageMgr.send(sender, Lists.newArrayList(participants),
					text);
	...
	public void setUp() {
		super.setUp();
		messageMgr = new MessageManagerImpl();

		final MemberRepository memberRepo = new MemberRepositoryDatastoreImpl();
		final ProfileRepository profileRepo = new ProfileRepositoryDatastoreImpl();
		final ThreadRepository threadRepo = new ThreadRepositoryDatastoreImpl();
		final MessageRepository messageRepo = new MessageRepositoryDatastoreImpl();
		final MemberThreadRepository memberThreadRepo = new MemberThreadRepositoryDatastoreImpl();
		final MemberMessageRepository memberMessageRepo = new MemberMessageRepositoryDatastoreImpl();
		final MessageTransformer messageTfr = new MessageTransformerImpl();
		final BlacklistManager blacklistMgr = new BlacklistManagerImpl();
		final QueueManager queueMgr = new TestQueueManager(messageMgr,
				memberRepo, threadRepo, messageRepo);

		setField(messageMgr, "memberRepository", memberRepo);
		setField(messageMgr, "profileRepository", profileRepo);
		setField(messageMgr, "threadRepository", threadRepo);
		setField(messageMgr, "memberThreadRepository", memberThreadRepo);
		setField(messageMgr, "memberMessageRepository", memberMessageRepo);
		setField(messageMgr, "messageRepository", messageRepo);
		setField(messageMgr, "messageTransformer", messageTfr);
		setField(messageMgr, "blacklistManager", blacklistMgr);
		setField(messageMgr, "queueManager", queueMgr);

		final BlacklistRepository blacklistRepo = new BlacklistRepositoryDatastoreImpl();
		setField(blacklistMgr, "blacklistRepository", blacklistRepo);
	}
	...
	private void testMessages(final Member sender, final String text,
			final Thread thread) {
		messageMgr.send(sender, thread, text);
		assertMessages(sender, thread, sender, text);
	}
